...
**❗❗❗❗极其重要:务必在安装面板后申请一个SSH证书，并且必须用https访问，如果是http协议将全程明文，数据包将全部被解包，节点死的一个比一个快**
...
```bash
https://你的服务器IP:端口/路径
例如：https://103.123.123.123:7890/purw
```
...
### 3.3 众生平等器——外套CloudflareCDN+优选IP

**❗❗❗提升极大❗❗❗**

可以把垃圾线路的速度提升到100mbps左右，基本是有20倍的提升，但也会把好线路的速度拉下来

#### 3.3.1 概述

凡事出必有因，CloudflareCDN的出现自然是解决问题的，现在假设一个电脑在英国，要访问Google，但Google的服务器在美国，物理上的距离就会导致延迟始终居高不下，这时候出现一个服务器叫CDN服务器，这个CDN服务器在英国，但是代理了Google.com，即如果要访问Google先访问CDN服务器，此时用户1用Google搜索了“番茄炒蛋怎么做”，Google返回了结果，CDN服务器就自动把结果下载下来，此时用户2再搜索“番茄炒蛋怎么做”，那么CDN服务器就不需要再访问Google.com，而是把直接返回之前下载的结果，延迟自然就低了，我们可以看一下他的链路变化，而这就是我们可以利用的地方

```
原始：用户 ---> google.com （英国-->美国）高延迟

套了CDN：用户1 ---> CDN服务器 --->Google.com （英国 --> 英国 --> 美国）高延迟

套了CDN：用户2 ---> CDN服务器 （英国 --> 英国）低延迟
```

我们利用的链路如下，用CDN服务器的线路代替我们的163出海线路

```
用户1 ---> CDN服务器 --->Google.com （英国 --> 英国 --> 美国）高延迟
```

 #### 3.3.2 如何利用--获取域名并把域名添加到Cloudflare

**域名：由于时效性的问题需要读者自行搜索注册教程，或者花点钱租一个域名**

1. **打开Cloudflare.com将注册好的域名填写进去，点击继续**
...
6. **将这两个ns服务器添加到你注册的域名中**
   ![图 12](images/1f0048b248a7adea92a9017523eff6f8471adf38d652489eb39870f716b5af87.png)
7. **等待一段时间DNS生效，在主机ping域名就可以看到CF给你分配的CDN IP了**



#### 3.3.3 如何利用--如何套到节点上

将IP改成CDN服务器IP，传输协议写WS，host填你的域名，注意这个节点的端口最好是常用的端口，如80或者443

此时节点应该是可用的，CF界面的代理状态应该是打开的，否则你ping域名只会是你的服务器ip，而不是分配的CDN ip

 ![image-20260719114043405](images\image-20260719114043405.png)

#### 3.3.4 如何利用--优选IP

不同的网络环境有不同的CDN服务器适配，一般自动分配的CDN服务器都比较糟糕，按照以下步骤优选CDN ip

1. 下载测速工具测速https://github.com/XIU2/CloudflareSpeedTest

2. 将延迟最低测速最高的的ip替换掉当前的IP即可
...
有商家可以从运营商手上拿到出海线路的使用权，而这些商家把线路拿出来卖，会给你一个IP和端口，在连接你的VPS之前，通过这个专线出海，再连接到你的VPS，可以保证在国内到出海口这段路径速度不慢，并且不用过GFW，所以使用ss协议就可以稳定（举个栗子：商家拿到上海-->日本的线路，商家得到就是一根光纤，一头在上海，一头在日本，从你家到上海，这是国内线路，不会风控，直接走他的光纤出国，出国直接接入国外骨干网，直接绕过GFW了，所以哪怕用再不安全的协议都能访问到），截止20206年，中转专线全线涨价，死的死亡的亡


#### 4.3.3 套一层cloudflare的CDN
